...
static const int GPS_TX = 17;   // GPS RXD -> TX2
static const int PIN_PPS_GPS = 35; // GPS PPS -> GPIO35 (solo entrada, OK)
...
void IRAM_ATTR ppsISR() {
  ...
  if (pps_seen) {
    pps_period_us = nowUs - pps_last_us;   // wrap-around de micros() se maneja bien con unsigned
  } else {
    pps_seen = true;
    pps_period_us = 0;
  }
...
void procesarSentencia(const String& line) {
  ...
  // Si querés ver TODAS las sentencias:
  // Serial.println(line);
}
...
void setup() {
  Serial.begin(115200);
  delay(800);
  Serial.println("\nIniciando ESP32 + DS3231 + GPS NMEA + PPS (boton=>guardar GPS valido UTC-3)...");
  ...
  Serial.println(" - Se loguea PPS para ver cuando aparece 1Hz.");
}
...
void loop() {
  ...
  if (ahora - ultimoPrint >= 1000) {
    ...
    noInterrupts();
    unsigned long pulsosSQW = sqw_pulsos;
    unsigned long pulsosPPS = pps_pulsos;
    unsigned long ppsPeriodUs = pps_period_us;
    bool ppsVisto = pps_seen;
    interrupts();

    DateTime now = rtc.now();

    Serial.printf("Hora RTC local: %04d/%02d/%02d %02d:%02d:%02d | SQW:%lu | PPS:%lu",
                  now.year(), now.month(), now.day(),
                  now.hour(), now.minute(), now.second(),
                  pulsosSQW, pulsosPPS);

    if (ppsVisto) {
      Serial.printf(" (T=%lu us)", ppsPeriodUs);
    } else {
      Serial.print(" (sin PPS)");
    }

    Serial.printf(" | TXT:%lu RMC:%lu GGA:%lu OTR:%lu",
                  cntTXT, cntRMC, cntGGA, cntOTROS);
